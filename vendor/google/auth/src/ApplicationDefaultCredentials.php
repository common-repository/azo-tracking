...
class ApplicationDefaultCredentials
{
    ...
    {
        $creds = null;
        $jsonKey = \Google\Auth\CredentialsLoader::fromEnv() ?: \Google\Auth\CredentialsLoader::fromWellKnownFile();
        ...
        if (!\is_null($jsonKey)) {
            ...
            $creds = \Google\Auth\CredentialsLoader::makeCredentials($scope, $jsonKey, $defaultScope);
        ...
        if (!\is_null($cache)) {
            $creds = new \Google\Auth\FetchAuthTokenCache($creds, $cacheConfig, $cache);
        }
        return $creds;
    }
    /**
    ...
    private static function onGce(callable $httpHandler = null, array $cacheConfig = null, CacheItemPoolInterface $cache = null)
    {
        ...
        return (new \Google\Auth\GCECache($gceCacheConfig, $cache))->onGce($httpHandler);
    }
}
