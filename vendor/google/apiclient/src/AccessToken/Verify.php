...
use AZO\Firebase\JWT\ExpiredException as ExpiredExceptionV3;
use AZO\Firebase\JWT\SignatureInvalidException;
use AZO\GuzzleHttp\Client;
use AZO\GuzzleHttp\ClientInterface;
use AZO\phpseclib3\Crypt\PublicKeyLoader;
use AZO\phpseclib3\Crypt\RSA\PublicKey;
use AZO\Psr\Cache\CacheItemPoolInterface;
use Google\Auth\Cache\MemoryCacheItemPool;
use Google\Exception as GoogleException;
use AZO\Stash\Driver\FileSystem;
use AZO\Stash\Pool;
use DateTime;
use DomainException;
use Exception;
use AZO\ExpiredException;
// Firebase v2
use LogicException;
/**
...
class Verify
{
    ...
    private function getFederatedSignOnCerts()
    {
        ...
        if (!isset($certs['keys'])) {
            throw new \Google\AccessToken\InvalidArgumentException('federated sign-on certs expects "keys" to be set');
        }
        return $certs['keys'];
    }
    ...
    {
        $jwtClass = 'JWT';
        if (\class_exists('AZO\\Firebase\\JWT\\JWT')) {
            $jwtClass = 'AZO\\Firebase\\JWT\\JWT';
        }
    ...
    private function getPublicKey($cert)
    {
        ...
        if (\class_exists('AZO\\phpseclib3\\Crypt\\RSA\\PublicKey')) {
    ...
    {
        if (\class_exists('AZO\\phpseclib3\\Crypt\\RSA')) {
            return 'AZO\\phpseclib3\\Crypt\\RSA';
        }
        if (\class_exists('AZO\\phpseclib\\Crypt\\RSA')) {
            return 'AZO\\phpseclib\\Crypt\\RSA';
        }
        return 'Crypt_RSA';
    }
    private function getBigIntClass()
    {
        if (\class_exists('AZO\\phpseclib3\\Math\\BigInteger')) {
            return 'AZO\\phpseclib3\\Math\\BigInteger';
        }
        if (\class_exists('AZO\\phpseclib\\Math\\BigInteger')) {
            return 'AZO\\phpseclib\\Math\\BigInteger';
        }
        return 'Math_BigInteger';
    }
    private function getOpenSslConstant()
    {
        if (\class_exists('AZO\\phpseclib3\\Crypt\\AES')) {
            return 'AZO\\phpseclib3\\Crypt\\AES::ENGINE_OPENSSL';
        }
        if (\class_exists('AZO\\phpseclib\\Crypt\\RSA')) {
            return 'AZO\\phpseclib\\Crypt\\RSA::MODE_OPENSSL';
        }
        if (\class_exists('AZO\\Crypt_RSA')) {
